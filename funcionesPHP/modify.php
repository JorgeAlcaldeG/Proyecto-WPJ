<?php
function modifyAlu($id){
    echo'<script  src="../js/valid_NewUserAlu.js"></script>';
    include '../proc/conexion.php';
    $sql = "SELECT * FROM tbl_alumne WHERE id_alumne={$id}";
    $sqlcurso = "SELECT id_classe, codi_classe FROM tbl_classe";
    $result = mysqli_query($connection, $sql);
    $user = mysqli_fetch_array($result, MYSQLI_ASSOC);
    echo"
    <form action='../proc/update.php' method='post'enctype='multipart/form-data' id='formulario'>
    <div class='user-box'>
        <input type='text' name='nombre' id='newusername' value='".$user['nom_alu']."'>
        <label>Nombre</label>
    </div>
    <div class='user-box'>
        <input type='text' name='apellido1' id='newuserape1' value='".$user['cognom1_alu']."'>
        <label>Apellido</label>
    </div>
    <div class='user-box'>
        <input type='text' name='apellido2' id='newuserape2' value='".$user['cognom2_alu']."'>
        <label>Segundo Apellido</label>
    </div>
    <div class='user-box'>
        <input type='text' name='dni' id='newuserdni' value='".$user['dni_alu']."'>
        <label>DNI/NIE</label>
    </div>
    <div class='user-box'>
        <input type='email' name='correo' id='newusermail' value='".$user['email_alu']."'>
        <label>email</label>
    </div>
    <div class='user-box ax'>
        <input type='text' name='telefono' id='newusertel' value='".$user['telf_alu']."'>
        <label>Telefono</label>
    </div>
    <div class='user-box'>
    <p class='active'>Curso</p>
    <select name='curso' class='form-select bg-transparent border-0 text-white border-bottom border-warning' >";
            $option = mysqli_query($connection, $sqlcurso);
            foreach($option as $curso){
                if ($curso['id_classe']== 5){
                }else{
                    echo "<option  value='{$curso['id_classe']}'>{$curso['codi_classe']}</option>";
                }
            }
    echo"</select>
    </div>

    <input type='file' name='foto' placeholder='Foto'>
    <br>
    <br>
    <input type='hidden' name='id' value='".$_GET['var']."'>
    <input type='hidden' name='typeuser' value='alu'>
    <input type='submit' value='Modificar' class='btn'>
</form>";
}
function modifyProf($id){
    echo'<script  src="../js/valid_NewUserProf.js"></script>';
    include '../proc/conexion.php';
    $sql = "SELECT * FROM tbl_professor WHERE id_professor={$id}";
    $sqlcurso = "SELECT id_dept, nom_dept FROM tbl_dept";
    $result = mysqli_query($connection, $sql);
    $user = mysqli_fetch_array($result, MYSQLI_ASSOC);
    echo"
    <form action='../proc/update.php' method='post'enctype='multipart/form-data' id='formulario'>
    <div class='user-box'>
        <input type='text' name='nombre' id='newusername' value='".$user['nom_prof']."'>
        <label>Nombre</label>
    </div>
    <div class='user-box'>
        <input type='text' name='apellido1' id='newuserape1'  value='".$user['cognom1_prof']."'>
        <label>Apellido</label>
    </div>
    <div class='user-box'>
        <input type='text' name='apellido2' id='newuserape2' value='".$user['cognom2_prof']."'>
        <label>Segundo Apellido</label>
    </div>
    <div class='user-box'>
        <input type='email' name='correo' id='newusermail' value='".$user['email_prof']."'>
        <label>email</label>
    </div>
    <div class='user-box ax'>
        <input type='text' name='telefono' id='newusertel' value='".$user['telf']."'>
        <label>Telefono</label>
    </div>
    <div class='user-box'>
    <p class='active'>Curso</p>
    <select name='curso' class='form-select bg-transparent border-0 text-white border-bottom border-warning'>";
            $option = mysqli_query($connection, $sqlcurso);
            foreach($option as $curso){
                echo "<option value='{$curso["id_dept"]}'>{$curso["nom_dept"]}</option>";
            }
    echo"</select>
    </div>

    <input type='file' name='foto' placeholder='Foto'>
    <br>
    <br>
    <input type='hidden' name='id' value='".$_GET['var']."'>
    <input type='hidden' name='typeuser' value='prof'>
    <input type='submit' value='Modificar' class='btn' dissable>
</form>";
}